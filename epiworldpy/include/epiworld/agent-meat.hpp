...
inline Agent<TSeq>::Agent(Agent<TSeq> &&p)
	: neighbors(std::move(p.neighbors)),
	  ...
	  state(p.state), state_prev(p.state_prev),
...
inline Agent<TSeq>::Agent(const Agent<TSeq> &p)
	: neighbors(nullptr), neighbors_locations(nullptr),
	...
	for (size_t i = 0u; i < p.get_n_tools(); ++i) {
		tools.emplace_back(
			std::shared_ptr<Tool<TSeq>>(p.tools[i]->clone_ptr()));
...
inline Agent<TSeq> &Agent<TSeq>::operator=(const Agent<TSeq> &other_agent) {
	...
	for (size_t i = 0u; i < other_agent.get_n_tools(); ++i) {
		tools.emplace_back(
			std::shared_ptr<Tool<TSeq>>(other_agent.tools[i]->clone_ptr()));
		tools.back()->set_agent(this, i);
	}
...
template <typename TSeq>
inline void Agent<TSeq>::add_tool(Model<TSeq> &model, const Tool<TSeq> &tool,
	...
	model._add_event(this, nullptr, tool_ptr, nullptr, state_new, queue,
					 EventAction::AddTool);
}

template <typename TSeq>
inline void Agent<TSeq>::set_virus(Model<TSeq> &model, const Virus<TSeq> &virus,
								   epiworld_fast_int state_new,
								   epiworld_fast_int queue) {
	...
	model._add_event(this, virus_ptr, nullptr, nullptr, state_new, queue,
					 EventAction::AddVirus);
}

template <typename TSeq>
inline void Agent<TSeq>::add_entity(Model<TSeq> &model, Entity<TSeq> &entity,
									epiworld_fast_int state_new,
									epiworld_fast_int queue) {

	model._add_event(this, nullptr, nullptr, &entity, state_new, queue,
					 EventAction::AddEntity);
}

template <typename TSeq>
inline void
Agent<TSeq>::rm_tool(Model<TSeq> &model, epiworld_fast_uint tool_idx,
					 epiworld_fast_int state_new, epiworld_fast_int queue) {

	if (tool_idx >= tools.size())
		throw std::range_error("The Tool you want to remove is out of range. "
							   "This Agent only has " +
							   std::to_string(tools.size()) + " tools.");

	model._add_event(this, nullptr, tools[tool_idx], nullptr, state_new, queue,
					 EventAction::RemoveTool);
}

template <typename TSeq>
inline void Agent<TSeq>::rm_tool(Model<TSeq> &model, ToolPtr<TSeq> &tool,
								 epiworld_fast_int state_new,
								 epiworld_fast_int queue) {
	...
	model._add_event(this, nullptr, tool, nullptr, state_new, queue,
					 EventAction::RemoveTool);
}
...
inline void Agent<TSeq>::rm_virus(Model<TSeq> &model,
	...
	model._add_event(this, virus, nullptr, nullptr, state_new, queue,
					 EventAction::RemoveVirus);
}

template <typename TSeq>
inline void
Agent<TSeq>::rm_entity(Model<TSeq> &model, epiworld_fast_uint entity_idx,
					   epiworld_fast_int state_new, epiworld_fast_int queue) {

	if (entity_idx >= entities.size())
		throw std::range_error("The Entity you want to remove is out of range. "
							   "This Agent only has " +
							   std::to_string(entities.size()) +
							   " entity(ies).");
	else if (entities.size() == 0u)
		throw std::logic_error("There is no entity to remove here!");

	model._add_event(this, nullptr, nullptr,
					 &model.get_entity(entities[entity_idx]), state_new, queue,
					 EventAction::RemoveEntity);
}

template <typename TSeq>
inline void Agent<TSeq>::rm_entity(Model<TSeq> &model, Entity<TSeq> &entity,
								   epiworld_fast_int state_new,
								   epiworld_fast_int queue) {
	...
	bool found =
		std::find(entities.begin(), entities.end(),
				  static_cast<size_t>(entity.get_id())) != entities.end();
	...
	model._add_event(this, nullptr, nullptr, &model.get_entity(entity.get_id()),
					 state_new, queue, EventAction::RemoveEntity);
}
...
inline ToolPtr<TSeq> &Agent<TSeq>::get_tool(std::string name) {
	...
	throw std::logic_error("The agent does not have a tool with name: " + name);
}
...
template <typename TSeq> inline void Agent<TSeq>::mutate_virus() {
	...
	// mutate_virus() requires the model context; skip if unavailable
	// (mutation callbacks are typically triggered internally by the model loop)
}
...
inline void Agent<TSeq>::swap_neighbors(Agent<TSeq> &other, size_t n_this,
										size_t n_other, Model<TSeq> &model) {

	if (n_this >= n_neighbors)
		throw std::range_error("The neighbor you want to swap is out of range. "
							   "This Agent only has " +
							   std::to_string(n_neighbors) + " neighbors.");
	if (n_other >= other.n_neighbors)
		throw std::range_error("The neighbor you want to swap is out of range. "
							   "This Agent only has " +
							   std::to_string(other.n_neighbors) +
							   " neighbors.");
...
template <typename TSeq>
inline std::vector<Agent<TSeq> *>
Agent<TSeq>::get_neighbors(Model<TSeq> &model) {
...
inline void Agent<TSeq>::change_state(Model<TSeq> &model,
	...
	model._add_event(this, nullptr, nullptr, nullptr, new_state, queue,
					 EventAction::ChangeState);

	return;
}

template <typename TSeq> inline unsigned int Agent<TSeq>::get_state() const {
	return state;
}
...
template <typename TSeq>
inline bool Agent<TSeq>::has_entity(std::string_view name,
									const Model<TSeq> &model) const {
...
template <typename TSeq>
inline const Entity<TSeq> &
Agent<TSeq>::get_entity(size_t i, const Model<TSeq> &model) const {
	if (entities.size() == 0)
...
inline bool Agent<TSeq>::operator==(const Agent<TSeq> &other) const {
	...
	EPI_DEBUG_FAIL_AT_TRUE(entities.size() != other.entities.size(),
	...
	EPI_DEBUG_FAIL_AT_TRUE(tools.size() != other.tools.size(),
